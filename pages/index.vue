    ...
    <UPageBody class="m-0 p-0">
        ...
        <UContainer :ui="{'constrained': 'max-w-3xl'}"
          ...
          <Center>
            <Headline class="pb-5" type="h1">{{ t('make.we') }}<br><span
                class="text-jm-primary-green uppercase">{{ t('make.simple') }}</span> <b class="uppercase">digital</b>.
            </Headline>
          ...
          <Center>
            <Headline type="h3">{{ t('office.your') }} <span class="lowercase">{{ t('office.for') }}</span>
              <b>{{ t('office.development') }}</b></Headline>
            <NuxtLink :to="{ path: '/', hash: '#machen' }" @click="scrollTo">
      ...
      <div class="relative bg-jm-primary-green">
        ...
        <UContainer id="machen" :ui="{'constrained': 'max-w-3xl'}" class="relative py-10">
          <Headline type="h6">{{ t('get.what') }} <b class="uppercase">{{ t('get.fromUs') }}</b></Headline>
          <Headline type="h4"><b>{{ t('get.time') }}</b>{{ t('get.energy') }} <b>{{ t('get.success') }}</b>
            {{ t('get.growth') }} <b>{{ t('get.efficiency') }}</b> {{ t('get.business') }}
          </Headline>
        </UContainer>
      ...
      <UContainer :ui="{'constrained': 'max-w-2xl'}" class="relative py-10">
        <Headline type="h6">{{ t('question.important') }} <b class="uppercase">{{ t('question.question') }}</b>
        </Headline>
        <Headline class="font-[400]" type="h2">„<b class="uppercase">{{ t('question.what') }}</b>
          {{ t('question.exactly') }} <b class="uppercase">{{ t('question.need') }}</b>“
        </Headline>
      </UContainer>

      <UContainer :ui="{'constrained': 'max-w-2xl'}" class="relative py-10">
        <Headline type="h6">{{ t('work.we') }} <b class="uppercase">{{ t('work.forYou') }}</b></Headline>
        <Headline class="text-jm-primary-brown" type="h5"><b>{{ t('work.honesty') }}</b></Headline>
        <Paragraph>{{ t('solution.problem') }} <b>{{ t('solution.solution') }}</b> {{ t('solution.priority') }}
          {{ t('solution.analyze') }} <b>{{ t('solution.conception') }}</b> {{ t('solution.products') }}
          {{ t('solution.equal') }} {{ t('solution.sides') }}
        </Paragraph>
        <Center>
      ...
      <div class="relative bg-jm-primary-green mt-64">
        ...
        <UContainer :ui="{'constrained': 'max-w-2xl'}" class="relative py-10">
          <Headline type="h6">{{ t('we.do') }} <b class="uppercase">{{ t('we.forYou') }}</b></Headline>
          <Headline class="font-[400]" type="h3">{{ t('we.development') }} <b
              class="text-jm-primary-brown">{{ t('we.digital') }}</b> <b>{{ t('we.customise') }}</b></Headline>
        </UContainer>
          ...
          <UBlogPost v-for="(article, index) in articles" :key="index" class="bg-jm-secondary-grey-lighter">
            <NuxtLink :to="article._path">
              ...
              <section class="px-3 pb-3">
                <Paragraph class="mt-3 mb-2 text-sm font-light">{{ article.date }} {{ t('by') }} <b
